...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 rounded-full font-semibold uppercase tracking-wider transition-all focus:outline-none disabled:opacity-50 disabled:pointer-events-none cursor-pointer",
  {
    variants: {
      variant: {
        default: "bg-brand-primary text-white shadow-md hover:bg-brand-primary/90 hover:scale-105 active:scale-95",
        outline: "border border-brand-primary text-brand-primary hover:bg-brand-primary/10",
        ghost: "text-brand-primary hover:text-brand-light transition-colors",
      },
      size: {
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, href, isExternal, iconLeft, iconRight, children, ...props }, ref) => {
    const content = (
      <>
        {iconLeft}
        <span>{children}</span>
